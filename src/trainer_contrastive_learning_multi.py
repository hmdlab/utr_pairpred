...
def load_split_dataset(cfg: AttrDict) -> dict:
    ...
    for phase, pair_list in pair_set_dict.items():
        ...
        if phase == "test":
            dataset_dict[phase] = dataset_class_test(seq_embs, pair_list)
        else:
...
class Trainer:
    ...
    def test_iteration(self, phase="test"):
        ...
        for embs, labels, pair_idx in tqdm(self.dataloader_dict[phase]):
            inputs = (embs[0].to(self.device), embs[1].to(self.device))
            logits, features = self.model.predict(inputs)
            ...
            if preds is None:
                ...
                utr5_features = np.append(
                    utr5_features, features[0].detach().cpu().numpy()
                )
                utr3_features = np.append(
                    utr3_features, features[1].detach().cpu().numpy()
                )
        ...
        return (
            scores,
            (out_cos_sim, out_logits, pair_idx_list),
            (utr5_features, utr3_features),
        )
    ...
    def test(self):
        ...
        scores, pred_results, features = self.test_iteration(phase="test")
        ...
        with open(os.path.join(self.cfg.result_dir, "output_features.pkl"), "wb") as f:
