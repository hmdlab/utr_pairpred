...
from dataset import PairDatasetAttn
...
class Trainer:
    ...
    def iterate(self, epoch: int, phase: str):
        ...
        for embs, labels, pair_idx in tqdm(
            self.dataloader_dict[phase], desc=f"Epoch: {epoch}"
        ):
            # inputs = torch.concat([embs[0], embs[1]], dim=1).to(self.device)
    ...
    def test(self):
        ...
        _, out_labels, preds, out_logits, scores, pair_idx_list = self.iterate(
            epoch=0, phase="test"
        )
